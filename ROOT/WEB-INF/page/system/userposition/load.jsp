...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			jQuery('#form_userposition_load').validationEngine();
	...
	<div class="tm_main">
		...
		<div class="tm_container">
			<ul class="tm_breadcrumb">
				...
				<li class="active"><tomtag:Message key="txt.sys.userposition.update" /></li>
			</ul>
		</div>
        
        <div class="tm_container">
        	<div class="tm_navtitle">
				<h1><tomtag:Message key="txt.sys.userposition.update" /></h1>
                <span><tomtag:Message key="txt.sys.userposition.updatetip" /></span>
            </div>
        ...
        <br/>
        <div class="tm_container">
			<form action="system/userposition/update.do" method="post" id="form_userposition_load">
        	<table width="100%" cellpadding="5" border="0" class="tm_table_form">
            	<tbody>
                    <tr>
                        <th width="120"><tomtag:Message key="txt.sys.userposition.fields.name" /> : </th>
                        <td>
							<input type="text" name="p_name" class="validate[required] tm_txt" size="50" maxlength="30" value="${userposition.p_name}" />
                    ...
                    <tr>
                        ...
                        <td>
                            <select class="tm_select" name="p_status" style="min-width:150px;">
                                <option value="1" <c:if test="${userposition.p_status == '1'}">selected</c:if>><tomtag:Message key="txt.other.status.normal" /></option>
                                <option value="-1" <c:if test="${userposition.p_status == '-1'}">selected</c:if>><tomtag:Message key="txt.other.status.lock" /></option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        <th valign="top"><tomtag:Message key="txt.sys.userposition.fields.remark" /> : </th>
                        <td><input type="text" name="p_remark" class="tm_txt" size="50" maxlength="50" value="${userposition.p_remark}" /></td>
                    </tr>
                </tbody>
			...
			<input type="hidden" name="p_id" value="${userposition.p_id}" />
			<input type="hidden" name="p_modifor" value="${sessionScope.SEN_USERNAME}" />
