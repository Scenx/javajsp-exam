...
<head>
	...
	<title><tomtag:Message key="txt.sys.course.teacher.update" /></title>
	...
	<script type="text/javascript">
		$(document).ready(function() { 
			jQuery('#form_teacher_load').validationEngine();
	...
	<div class="tm_main">
		...
		<div class="tm_container">
			<ul class="tm_breadcrumb">
				...
				<li class="active"><tomtag:Message key="txt.sys.course.teacher.update" /></li>
			</ul>
		</div>
        
        <div class="tm_container">
        	<div class="tm_navtitle">
				<h1><tomtag:Message key="txt.sys.course.teacher.update" /></h1>
                <span><tomtag:Message key="txt.sys.course.teacher.updatetip" /></span>
            </div>
        ...
        <br/>
        <div class="tm_container">
			<form action="system/course/teacher/update.do" method="post" id="form_teacher_load">
        	<table width="100%" cellpadding="5" border="0" class="tm_table_form">
            	<tbody>
                   <tr>
                        <th width="120"><tomtag:Message key="txt.sys.course.teacher.fields.name" /> : </th>
                        <td>
							<input type="text" name="t_name" class="validate[required] tm_txt" size="50" maxlength="30" value="${teacher.t_name}" />
							<span class="tm_required">*</span>
						</td>
                    </tr>
					<tr>
                        <th><tomtag:Message key="txt.sys.course.teacher.fields.phone" /> : </th>
                        <td><input type="text" name="t_phone" class="tm_txt" size="50" maxlength="30" value="${teacher.t_phone}" /></td>
                    </tr>
					<tr>
                        <th><tomtag:Message key="txt.sys.course.teacher.fields.email" /> : </th>
                        <td><input type="text" name="t_email" class="tm_txt" size="50" maxlength="30" value="${teacher.t_email}" /></td>
                    </tr>
					<tr>
                        ...
                        <td>
							<c:choose>
								<c:when test="${not empty teacher.t_photo}">
									<div class="tm_img_preview"><img src="${tm_base}${teacher.t_photo}" style="cursor:pointer" onclick="tm_showPic(this.src)" /><a href="javascript:;" onclick="tm_removePhoto('t_photo')"></a></div>
									<input type="button" id="btn_photo" />
									<input type="hidden" name="t_photo" id="t_photo" value="${teacher.t_photo}" />
								</c:when>
								<c:otherwise>
					...
					<tr>
                        ...
                        <td>
							<textarea name="t_info" rows="5" cols="50" class="tm_txtx">${teacher.t_info}</textarea>
						</td>
                    </tr>
                </tbody>
