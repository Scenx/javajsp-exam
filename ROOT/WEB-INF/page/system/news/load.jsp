...
<head>
	...
	<title><tomtag:Message key="txt.sys.news.update" /></title>
	...
	<script type="text/javascript">
		...
		window.onload = function(){
			...
			jQuery('#form_news_update').validationEngine();
	...
	<div class="tm_main">
        ...
        <div class="tm_container">
        	<div class="tm_navtitle">
				<h1><tomtag:Message key="txt.sys.news.update" /></h1>
                <span><tomtag:Message key="txt.sys.news.updatetip" /></span>
            </div>
        ...
        <br/>
        <div class="tm_container">
			<form action="system/news/update.do" method="post" id="form_news_update">
        	<table width="100%" cellpadding="5" border="0" class="tm_table_form">
            	<tbody>
					<tr>
                        <th width="120"><tomtag:Message key="txt.sys.news.fields.title" /> : </th>
                        <td colspan="3">
							<input type="text" name="n_title" class="validate[required] tm_txt" size="80" maxlength="50" value="${news.n_title}" />
							<input id="n_title_color" name="n_title_color" type="text" class="input_cxcolor" readonly value="${news.n_title_color}" />
						</td>
                    </tr>
					<tr>
                        ...
                        <td colspan="3">
							<c:choose>
								<c:when test="${not empty news.n_photo}">
									<div class="tm_img_preview"><img src="${tm_base}${news.n_photo}"style="cursor:pointer" onclick="tm_showPic(this.src)" /><a href="javascript:;" onclick="tm_removePhoto('n_photo')"></a></div>
									<input type="button" id="btn_photo" />
									<input type="hidden" name="n_photo" id="n_photo" value="${news.n_photo}" />
								</c:when>
								<c:otherwise>
                    ...
                    <tr>
                        ...
                        <td width="40%">
							<select class="tm_select" name="n_classid" style="min-width:200px">
								...
								<option value="${category.c_id}" <c:if test="${news.n_classid == category.c_id}">selected</c:if>>${category.c_name}</option>
								</c:forEach>
							</select>
						</td>
                        ...
                        <td width="40%">
							<select class="tm_select" name="n_status" style="min-width:200px">
                                <option value="1" <c:if test="${news.n_status == '1'}">selected</c:if>><tomtag:Message key="txt.other.status.open" /></option>
                                <option value="0" <c:if test="${news.n_status == '0'}">selected</c:if>><tomtag:Message key="txt.other.status.close" /></option>
                            </select>
					...
					<tr>
                        ...
                        <td>
							<select class="tm_select" name="n_totop" style="min-width:200px">
                                <option value="0" <c:if test="${news.n_totop == '0'}">selected</c:if>><tomtag:Message key="txt.sys.news.fields.top.n" /></option>
                                <option value="1" <c:if test="${news.n_totop == '1'}">selected</c:if>><tomtag:Message key="txt.sys.news.fields.top.y" /></option>
                            </select>
						</td>
						<th><tomtag:Message key="txt.sys.news.fields.visit" /> : </th>
                        <td>
							<input type="text" name="n_visit" class="validate[required,custom[integer]] tm_txt" size="50" value="${news.n_visit}" />
						</td>
                    </tr>
					<tr>
                        <th><tomtag:Message key="txt.sys.news.fields.author" /> : </th>
                        <td>
							<input type="text" name="n_author" class="tm_txt" size="50" value="${news.n_author}" />
						</td>
						<th><tomtag:Message key="txt.sys.news.fields.from" /> : </th>
                        <td>
							<input type="text" name="n_newsfrom" class="tm_txt" size="50" value="${news.n_newsfrom}" />
						</td>
                    </tr>
					<tr>
                        <th><tomtag:Message key="txt.sys.news.fields.content" /> : </th>
                        <td colspan="3">
							<textarea name="n_content" rows="5" cols="40" class="validate[required]">${news.n_content}</textarea>
						</td>
                    </tr>
                </tbody>
			...
			<input type="hidden" name="n_modifyor" value="${sessionScope.SEN_USERNAME}" />
