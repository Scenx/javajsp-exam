...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			jQuery('#form_branch_load').validationEngine();
	...
	<div class="tm_main">
		...
		<div class="tm_container">
			<ul class="tm_breadcrumb">
				...
				<li class="active"><tomtag:Message key="txt.sys.branch.update" /></li>
			</ul>
		</div>
        
        <div class="tm_container">
        	<div class="tm_navtitle">
				<h1><tomtag:Message key="txt.sys.branch.update" /></h1>
                <span><tomtag:Message key="txt.sys.branch.updatetip" /></span>
            </div>
        ...
        <br/>
        <div class="tm_container">
			<form action="system/branch/update.do" method="post" id="form_branch_load">
        	<table width="100%" cellpadding="5" border="0" class="tm_table_form">
            	<tbody>
                    <tr>
                        <th width="120"><tomtag:Message key="txt.sys.branch.fields.name" /> : </th>
                        <td>
							<input type="text" name="b_name" class="validate[required] tm_txt" size="50" value="${branch.b_name}" maxlength="30" />
                    ...
                    <tr>
                        ...
                        <td>
                            <select class="tm_select" name="b_status" style="min-width:150px;">
                                <option value="1" <c:if test="${branch.b_status == '1'}">selected</c:if>><tomtag:Message key="txt.other.status.normal" /></option>
                                <option value="-1" <c:if test="${branch.b_status == '-1'}">selected</c:if>><tomtag:Message key="txt.other.status.lock" /></option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        <th valign="top"><tomtag:Message key="txt.sys.branch.fields.remark" /> : </th>
                        <td><input type="text" name="b_remark" class="tm_txt" size="50" value="${branch.b_remark}" maxlength="50" /></td>
                    </tr>
                </tbody>
			...
			<input type="hidden" name="b_id" value="${branch.b_id}" />
			<input type="hidden" name="b_pid" value="0" />
			<input type="hidden" name="b_order" value="1" />
			<input type="hidden" name="b_modifyor" value="${sessionScope.SEN_USERNAME}" />
