...
<head>
	...
	<title><tomtag:Message key="txt.sys.qdb.update" /></title>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			jQuery('#form_qdb_load').validationEngine();
	...
	<div class="tm_main">
		...
		<div class="tm_container">
			<ul class="tm_breadcrumb">
				...
				<li class="active"><tomtag:Message key="txt.sys.qdb.update" /></li>
			</ul>
		</div>
        
        <div class="tm_container">
        	<div class="tm_navtitle">
				<h1><tomtag:Message key="txt.sys.qdb.update" /></h1>
                <span><tomtag:Message key="txt.sys.qdb.updatetip" /></span>
            </div>
        ...
        <br/>
        <div class="tm_container">
			<form action="system/qdb/update.do" method="post" id="form_qdb_load">
        	<table width="100%" cellpadding="5" border="0" class="tm_table_form">
            	<tbody>
                    <tr>
                        <th width="120"><tomtag:Message key="txt.sys.qdb.fields.name" /> : </th>
                        <td>
							<input type="text" name="d_name" class="validate[required] tm_txt" size="50" maxlength="30" value="${qdb.d_name}" />
                        ...
                        <td>
							<c:choose>
								<c:when test="${not empty qdb.d_logo}">
									<div class="tm_img_preview"><img src="${tm_base}${qdb.d_logo}" style="cursor:pointer" onclick="tm_showPic(this.src)" /><a href="javascript:;" onclick="tm_removePhoto('d_logo')"></a></div>
									<input type="button" id="btn_photo" />
									<input type="hidden" name="d_logo" id="d_logo" value="${qdb.d_logo}" />
								</c:when>
								<c:otherwise>
					...
					<tr>
                        ...
                        <td>
							<select class="tm_select" name="d_status" style="width:150px">
                                <option value="1" <c:if test="${qdb.d_status == 1}">selected</c:if>><tomtag:Message key="txt.other.status.open" /></option>
                                <option value="0" <c:if test="${qdb.d_status == 0}">selected</c:if>><tomtag:Message key="txt.other.status.close" /></option>
                            </select>
					...
					<tr>
                        ...
                        <td><input type="text" name="d_remark" class="tm_txt" size="50" value="${qdb.d_remark}" /></td>
                    </tr>
                </tbody>
			...
			<input type="hidden" name="d_id" value="${qdb.d_id}" />
