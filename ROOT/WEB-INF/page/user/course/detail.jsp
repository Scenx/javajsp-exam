...
<head>
	...
	<script type="text/javascript">
		...
		function tm_loadlesson(chapterid, lessonid, filetype, fielpath, obj){
			if(fielpath.endWith("flv") || fielpath.endWith("mp3") || fielpath.endWith("mp4")){
				$(".tm_media").attr("href", "inc/jwplayer/player.swf?file=${tm_base}" + fielpath);
				$('.tm_media').media({ width:'100%', height:var_player_height, autoplay: true });
			}else{
				$(".tm_media").attr("href", "${tm_base}" + fielpath);
				$('.tm_media').media({ width:'100%', height:var_player_height, autoplay: true });
			}
...
<body oncontextmenu="return false">
						...
						<td width="500" valign="top" class="tm_course_list">
							...
							<ul class="tm_lessons">
								<c:forEach var="chapter" items="${course.chapters}">
									...
									<li><a href="javascript:void(0);" onclick="javascript:tm_loadlesson('${chapter.id}','${lesson.id}','${lesson.filetype}','${lesson.filepath}', this)">
											<h1>${lesson.name}</h1>
											<p>${lesson.remark}</p>
	...
	<div id="tm_teacherinfo" style="display:none">
		<table width="100%" cellpadding="5" border="1" class="tm_table_form">
			<tbody>
			<tr>
